...
import { Row, Col, Card, Icon, Avatar,List, Typography } from "antd";
...
class BookList extends Component {
  ...
  getCategory() {
    const listItems = category.map(cat => <li key={cat}><a>{cat}</a></li>);
    return listItems;
  }
  clickCategory=(item)=>{
            ...
            {/* <h3 style={{ marginBottom: 16 }}>Default Size</h3> */}
              <List
                ...
                renderItem={(item, index) => (
                  <List.Item>
                    <Typography.Text mark={index===markId}>{index+1}. </Typography.Text><a onClick={()=>this.clickCategory(index)}> {item}</a>
                  </List.Item>
                )}
              />
          </Col>
